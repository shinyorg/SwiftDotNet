...
/// </summary>
/// <remarks>
/// WPF exposes no theme API of its own (unlike WinUI's <c>ActualTheme</c> or MAUI's <c>AppTheme</c>), so
/// the setting is read from the same registry value the shell itself uses. It is <c>internal</c> because
/// the sibling WinForms host declares the same helper, and a consumer referencing both assemblies would
